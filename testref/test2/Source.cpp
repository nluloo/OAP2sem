...
int int_pwr(int m, int e); // Здесь передача параметров в функцию идет не через регистр. А через обычный параметр.
...
int int_pwr(int m, int e) { // Тело функции в которую передают данные через параметр

	int temp; // Объявление переменной типа int. Ключевое слово автоматически - auto
	temp = 1; // Присваиваем temp = 1;

	for (; e; e--) temp = temp * m; // Цикл будет выполняться 30 раз(пока e не станет 0).

	return temp; // Возврат получившегося значения.
}